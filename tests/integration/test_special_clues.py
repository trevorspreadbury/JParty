...
def test_image_question_paths_accept_and_clear_image(game_with_players):
...
def test_daily_double_load_and_wager_flow(game_with_players, monkeypatch):
    game = game_with_players
    question = game.current_round.get_question(0, 0)
    question.dd = True
...
def test_start_final_opens_wagers_and_lights_players(game_with_players):
    game = game_with_players
...
def test_final_judgement_order_blank_answer_and_scoring(game_with_players):
    game = game_with_players
    game.current_round = game.data.rounds[2]
...
def test_end_game_handles_tie_and_single_winner(game_with_players):
    game = game_with_players
