...
def test_check_internet_exits_when_request_fails(monkeypatch):
    critical_calls = []
    monkeypatch.setattr(main.requests, "get", lambda url: (_ for _ in ()).throw(main.requests.exceptions.ConnectionError()))
    monkeypatch.setattr(main.QMessageBox, "critical", lambda *args, **kwargs: critical_calls.append((args, kwargs)))
...
def test_check_second_monitor_exits_without_two_screens(monkeypatch):
    ...
    monkeypatch.setattr(main.QMessageBox, "critical", lambda *args, **kwargs: critical_calls.append((args, kwargs)))
    monkeypatch.setattr(main.sys, "exit", lambda code=0: (_ for _ in ()).throw(SystemExit(code)))
